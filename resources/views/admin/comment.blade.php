        ...
        <div class='comment_all px-2 py-4 border-bottom'><h5><span class="icon-user">用户：</span> <span style="color: #2c5786">{{$value->username}}</span> ：<small>{{$value->comment_content}}</small></h5>
            <div class='pt-3'>
                ...
                <div class='manage_button float-right'>
                    <a class="cheak" href="/article/read?id={{$value->article_id}} " >前往</a> | <a class="button_delete" href="/comment/delete/?id={{$value->id}}" onclick="javascript:stop_ajax();" style="color: #e9322d">删除</a>
                </div>
            </div>
        </div>
    @endforeach
